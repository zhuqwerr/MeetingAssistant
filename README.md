...
5. 左侧四个 Tab 展示即时摘要、决策结论、待办事项、AI 建议；即时摘要内按主题呈现讨论脉络，右侧显示转写。默认每 30 秒增量更新；听到关键事件时合并 8 秒内的触发，会议中每 15 分钟校正一次。校正前先补齐尚未总结的转写，失败时保留已成功保存的状态。可点“立即总结”，“查看原文”跳转到引用片段。

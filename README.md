...
MeetingAssistant 是一款面向 Windows 的本地会议助手。它可以同时采集麦克风与系统声音，使用 Whisper 在本机持续转写，并在会议进行过程中维护一份不断更新的完整纪要。用户可以随时查看关键要点、待办事项和 AI 建议，也可以直接向当前会议提问，并跳回对应原话核对。
...
| 结构化会议纪要 | 自动整理即时摘要、讨论主题、关键要点、待办事项和 AI 建议，未明确的负责人或时间不会被模型擅自补全。 |
...
5. 左侧四个 Tab 展示即时摘要、关键要点、待办事项、AI 建议；右侧显示转写。默认每 30 秒增量更新；听到关键事件时合并 8 秒内的触发，会议中每 15 分钟校正一次。校正前先补齐尚未总结的转写，失败时保留已成功保存的状态。可点“立即总结”，“查看原文”跳转到引用片段。

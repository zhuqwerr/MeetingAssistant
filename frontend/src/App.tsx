...
export default function App() {
  ...
  const start = () => action(async () => {
    const result = await api<{ id: string }>('/meetings', 'POST', { title: title.trim() || '未命名会议', source, language: selectedLanguage, microphone: microphone || null, speaker: speaker || null });
  ...
  return <div className="app-shell">
    ...
    <header className="topbar">
      <form className="header-controls" onSubmit={e => { e.preventDefault(); if (!active) void start(); }}>
        ...
        <label className="header-language"><span className="sr-only">转写语言</span><select value={active ? meeting?.language ?? selectedLanguage : selectedLanguage} onChange={e => setLanguage(e.target.value)} disabled={active}><option value="zh">中文</option><option value="en">English</option><option value="auto">自动识别</option></select></label>
        {active ? <button type="button" className="primary stop-button header-record-button" disabled={busy || status === 'stopping'} onClick={() => void stop()}>{status === 'stopping' ? <LoaderCircle size={17} className="spin"/> : <Square size={14} fill="currentColor"/>}{status === 'starting' ? '取消准备' : status === 'stopping' ? '正在收尾…' : '结束会议'}</button> : health?.model?.installed ? <button className="primary header-record-button" type="submit" disabled={busy || !!serviceError || !settings}>{busy ? <LoaderCircle className="spin" size={18}/> : <Mic size={18}/>}开始会议</button> : <button className="primary header-record-button" type="button" disabled={busy || !!serviceError || !settings || health?.model?.status === 'downloading'} onClick={() => void openSettings()}>{health?.model?.status === 'downloading' ? <LoaderCircle className="spin" size={18}/> : <Download size={18} />}{health?.model?.status === 'downloading' ? `模型下载中 ${health.model.progress}%` : '下载语音模型'}</button>}
      </form>
    </header>
    ...
    {modal === 'settings' && settings && <SettingsDialog settings={settings} health={health} devices={devices} microphone={microphone} speaker={speaker} setDevices={(mic, speaker) => { setMicrophone(mic); setSpeaker(speaker); }} onSaved={saved => { setSettings(saved); void refreshHealth(); }} onClose={() => setModal(null)}/>}
    </div>
  </div>;
}
